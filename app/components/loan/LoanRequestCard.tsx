...
export function LoanRequestCard({ request, completedLoans }: LoanRequestCardProps) {
  ...
  const handleInvest = () => writeContract({ abi: LoanMarketABI, address: LOAN_MARKET_ADDRESS, functionName: "fundLoan", args: [BigInt(request.id)], value: request.amountRequested });
  const handleWithdraw = () => writeContract({ abi: LoanMarketABI, address: LOAN_MARKET_ADDRESS, functionName: "withdrawAsBorrower", args: [BigInt(request.id)] });
  const handleRepay = () => writeContract({ abi: LoanMarketABI, address: LOAN_MARKET_ADDRESS, functionName: "repay", args: [BigInt(request.id)], value: repaymentAmount });
  const handleCancel = () => writeContract({ abi: LoanMarketABI, address: LOAN_MARKET_ADDRESS, functionName: "cancelLoan", args: [BigInt(request.id)] });
  const handleCancelFundedLoan = () => writeContract({ abi: LoanMarketABI, address: LOAN_MARKET_ADDRESS, functionName: "cancelFundedLoan", args: [BigInt(request.id)] });
      ...
      <div className="space-y-3">
        {isLoanOpenForInvestment && (
          <div className="space-y-2">
            {!aiAnalysis && <Button variant="outline" className="w-full" onClick={handleAiAnalysis} disabled={isAnalyzing}><Sparkles className="mr-2 h-4 w-4" />{isAnalyzing ? "Analyzing..." : "Analyze Risk with AI"}</Button>}
            ...
            {aiAnalysis && (
              <div className="p-3 bg-card rounded-md border text-center">
                <p className="text-sm font-bold">Risk Score: <span className="text-green-500">{aiAnalysis.riskScore}/100</span></p>
  ...
  const loanEndDate = useMemo(() => {
    ...
    return endDate.toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "2-digit", }) + ", " + endDate.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false });
  }, [request.status, request.startTimestamp, request.durationSecs]);
      ...
      <CardContent className="space-y-4 flex-grow flex flex-col">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <ScoreStars score={displayScore} completedLoans={completedLoans} isFinalScore={(request.status === 3 || request.status === 4) && request.score > 0} />
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
